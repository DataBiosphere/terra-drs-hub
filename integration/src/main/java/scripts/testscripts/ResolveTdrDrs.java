...
public class ResolveTdrDrs extends TestScript {
  // All URIs pont to the same TDR hosted DRS object. Some URIs are aliases created in the TDR
  // instance
  private static final List<String> TDR_TEST_DRS_URIS =
      List.of(
  ...
  public void userJourney(TestUserSpecification testUser) throws Exception {
    ...
    TDR_TEST_DRS_URIS.forEach(
        u -> {
          var actualMetadata = drsHubApi.resolveDrs(new RequestObject().url(u));
          ...
          Assertions.assertEquals(
              expectedMetadata, actualMetadata, "Metadata for drs uri %s is correct".formatted(u));
        });
  }
}
